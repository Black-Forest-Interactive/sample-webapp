...
import de.blackforrestdevelopment.sample.webapp.common.SimpleDataObjectConverter
import de.blackforrestdevelopment.sample.webapp.core.account.AccountStorage
...
import de.blackforrestdevelopment.sample.webapp.core.account.api.AccountDetails
...
class AccountStorageService(
    ...
    fun getInfoByIds(ids: Set<Long>): List<AccountInfo> {
        ...
        return accounts.map { AccountInfo.create(it) }
    }
    ...
    fun getDetailsByIds(ids: Set<Long>): List<AccountDetails> {
        val accounts = repository.findByIdIn(ids)
        return accounts.map { AccountDetails.create(it) }
    }
